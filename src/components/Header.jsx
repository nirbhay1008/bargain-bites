...
export const Header = () => {
  ...
  const [isMenu , setIsMenue] = useState(false);
  const login = async () => {
    if( !user){
      const {
  ...
  return (
    <header className="fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16">
            ...
            />
            {
                ...
                className="w-40 bg-grey-50 shadow-xl rounded-lg flex-col absolute top-12 right-0">
              {
                user && user.email === "nirbhaychopra1008@gmail.com" && (
                  <Link to = {'/createItem'}>
                  <p className="px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base">New item <MdAdd> </MdAdd>
                  </p>
                  </Link>
                )
              }
      ...
      {/* for logo */}
      <div className="flex items-ceter justify-between md:hidden w-full h-full">
      <Link to={"/"} className="flex items-center gap-2">
          <img
            src={Logo}
            className="w-13 h-20 object-cover"
